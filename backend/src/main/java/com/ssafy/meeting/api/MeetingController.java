...
import com.ssafy.meeting.api.dto.JoinRequest;
...
public class MeetingController {
    ...
    /** 참여: 열린 회의 재사용/생성 + 토큰 발급 (01, 02). */
    @PostMapping("/api/projects/{projectId}/meetings/join")
    public ApiResponse<JoinResponse> join(@PathVariable int projectId,
                                          @Valid @RequestBody JoinRequest request) {
        memberDirectory.remember(request.memberId(), request.nameOrDefault());
        JoinResponse response = meetingService.join(projectId, request.memberId(), request.nameOrDefault());
